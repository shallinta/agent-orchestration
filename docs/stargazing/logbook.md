...
| Stargazing 7 | 持久化、进程重启与不确定结果 | 已完成 | 有条件可行 |
| Stargazing 8 | 并发、取消、停用与保证强度 | 待开始 | 未决 |
| Stargazing 9 | 阶段综合结论 | 待开始 | 未决 |

## 2026-09-04：Stargazing 7 以“有条件可行”完成

用确定性假 Adapter、SQLite 和三个独立 Python 进程完成了“已接受、无可靠终态时 Harness 退出”的最小恢复实验。第一个进程只在 Adapter 接受账本和 Harness 接受事实都已落盘并能由新连接读回后以固定退出码终止；后两个进程分别执行首次与重复恢复。

最终结构证据为：两个角色、一项委托、一次执行尝试和一次 Adapter 接受均被恢复；执行保持`terminal=null`并得到`result_unknown`；Adapter 总调用一次、重复调用零次、自动重派零次；未知恢复事件恰好一条，二次恢复摘要和完整事实快照一致；没有 Agent 结果、角色消息或委托完成事实。21 项自动测试和手工端到端探针均通过。

独立审阅过程中还发现并修复了实验事实模型允许空终态和接受前终态的漏洞，失败测试先复现了“终态事件与结果未知同时存在”的矛盾，再由方法校验和 SQLite 约束共同封闭。

因此本项核心门槛成立，但结论限定为`有条件可行`：只覆盖确定性假 Adapter、已提交 SQLite 事务、顺序单 writer 和受控进程退出。更早的送达未知、可查询终态、真实 Agent/session、在途进程接管、进程与外部副作用状态、断电耐久、并发及生产容灾均未验证，不会为了关闭本项混入同一探针。Stargazing 8 尚未开始。

## 2026-09-04：开始 Stargazing 7

用户确认把“Adapter 已接受执行、Harness 已提交接受事实、可靠终态尚未产生时 Harness 退出”作为首轮最小崩溃窗口，并授权持续推进至 Stargazing 7 形成结论。首轮只验证新进程能否恢复已确认事实、将该执行诚实归为`结果未知`、禁止自动重派且保持幂等，不混入真实 Agent、外部副作用、在途进程接管、并发或生产数据库。

当前采用“确定性假 Adapter + SQLite + 三个真实 Python 进程”的可丢弃实验：第一个进程在接受事实提交后以固定退出码模拟崩溃，第二和第三个进程分别执行首次与重复恢复。SQLite 只用于证明显式提交的事实能跨进程读取，不代表正式存储选型，也不证明断电、多机、并发 writer 或高可用。

本项明确区分`结果未知`、`送达未知`、进程存活未知和外部副作用未知。Probe 7.1 通过后只按证据缺口决定是否追加送达窗口、可查询终态或真实 CLI session 探针；当前尚未编写或运行实验，结论仍为`未决`。
