...
项目处于长期、细粒度的 Dreaming 阶段。目前只建立产品认识、项目记录与协作规则，尚未进入版本范围、技术方案或功能实现，也未确定编程语言、运行时、存储方案或 UI 技术实现。

当前工作重点是从核心问题开始，逐项确认理想使用场景、产品概念、责任边界和真实未决问题。只有用户明确提出进入版本规划后，才开始讨论从 `v0.0.1` 起的具体迭代范围。
